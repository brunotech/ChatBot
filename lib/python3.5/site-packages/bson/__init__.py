...
def _get_binary(data, position, obj_end, opts, dummy1):
    ...
    if subtype in (3, 4):
        ...
        if uuid_representation == JAVA_LEGACY:
            ...
            value = uuid.UUID(bytes=java[:8][::-1] + java[8:16][::-1])
...
def _elements_to_dict(data, position, obj_end, opts):
    ...
    pos = position
    for key, value, pos in _iterate_elements(data, pos, obj_end, opts):
        result[key] = value
    if pos != obj_end:
...
def _encode_uuid(name, value, dummy, opts):
    ...
    if uuid_representation == OLD_UUID_SUBTYPE:
        ...
        data = from_uuid[:8][::-1] + from_uuid[8:16][::-1]
...
def _name_value_to_bson(name, value, check_keys, opts):
    ...
    raise InvalidDocument(f"cannot convert value of type {type(value)} to bson")
    ...
    try:
        ...
        if top_level and "_id" in doc:
            elements.append(_name_value_to_bson(b"_id\x00", doc["_id"],
                                                check_keys, opts))
        elements.extend(
            _element_to_bson(key, value, check_keys, opts)
