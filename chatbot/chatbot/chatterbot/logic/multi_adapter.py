        ...
        for adapter in self.get_adapters():
            if adapter.can_process(statement):
                ...
                if type(output) == tuple:
                    warnings.warn(
                        f'{adapter.class_name} returned two values when just a Statement object was expected. You should update your logic adapter to return just the Statement object. Make sure that statement.confidence is being set.',
                        DeprecationWarning,
                    )
                    output = output[1]       
